...
class FieldConfig {
  ...
  static List<FieldConfig> defaults() => const [
    ...
    FieldConfig(field: TimeEntryField.description, isVisible: true, order: 2),
    FieldConfig(field: TimeEntryField.issue, isVisible: true, order: 3),
    FieldConfig(field: TimeEntryField.repository, isVisible: false, order: 4),
  ...
  static List<FieldConfig> decode(String jsonStr) {
    try {
      ...
      final extras = defaults()
          ...
          .toList();
      return [...loaded, ...extras]..sort((a, b) => a.order.compareTo(b.order));
    } catch (_) {
      return defaults();
    }
  }
