...
class SupabaseClientRepository implements ClientRepository {
  ...
  Future<Client> getClient(String id) async {
    final row = await _client.from('clients').select().eq('id', id).single();
    return _fromRow(row);
  }
  ...
  Future<String> insertClient(ClientsCompanion companion) async {
    ...
    await _client.from('clients').insert({
      ...
      if (companion.contactName.present)
        'contact_name': companion.contactName.value,
      ...
      if (companion.addressLine1.present)
        'address_line1': companion.addressLine1.value,
      if (companion.addressLine2.present)
        'address_line2': companion.addressLine2.value,
      if (companion.city.present) 'city': companion.city.value,
      if (companion.stateProvince.present)
        'state_province': companion.stateProvince.value,
      if (companion.postalCode.present)
        'postal_code': companion.postalCode.value,
      if (companion.country.present) 'country': companion.country.value,
      if (companion.hourlyRate.present)
        'hourly_rate': companion.hourlyRate.value,
      ...
      if (companion.defaultTemplateId.present)
        'default_template_id': companion.defaultTemplateId.value,
      if (companion.paymentTermsOverride.present)
        'payment_terms_override': companion.paymentTermsOverride.value,
      if (companion.paymentTermsDaysOverride.present)
        'payment_terms_days_override': companion.paymentTermsDaysOverride.value,
  ...
  Future<bool> updateClient(String id, ClientsCompanion companion) async {
    ...
    if (companion.contactName.present)
      map['contact_name'] = companion.contactName.value;
    ...
    if (companion.hourlyRate.present)
      map['hourly_rate'] = companion.hourlyRate.value;
    ...
    if (companion.paymentTermsOverride.present)
      map['payment_terms_override'] = companion.paymentTermsOverride.value;
    if (companion.paymentTermsDaysOverride.present)
      map['payment_terms_days_override'] =
          companion.paymentTermsDaysOverride.value;
  ...
  Future<double> getTotalBilled(String clientId) async {
    ...
    return rows.fold<double>(
      0,
      (sum, r) => sum + ((r['total'] as num?) ?? 0).toDouble(),
    );
  }
  ...
  Future<double> getTotalPaid(String clientId) async {
    ...
    return rows.fold<double>(
      0,
      (sum, r) => sum + ((r['amount_paid'] as num?) ?? 0).toDouble(),
    );
  }
}
