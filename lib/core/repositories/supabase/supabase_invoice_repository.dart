...
class SupabaseInvoiceRepository implements InvoiceRepository {
  ...
  Future<Invoice> getInvoice(String id) async {
    final row = await _client.from('invoices').select().eq('id', id).single();
  ...
  Future<String> createInvoice({
    ...
    await _client.from('invoices').insert({
      ...
      'late_fee_amount': invoice.lateFeeAmount.present
          ? invoice.lateFeeAmount.value
          : 0.0,
      ...
      'template_id': invoice.templateId.present
          ? invoice.templateId.value
          : null,
      'template_type': invoice.templateType.present
          ? invoice.templateType.value
          : 'detailed',
      'created_at': now,
      'updated_at': now,
    });
    ...
    for (var i = 0; i < lineItems.length; i++) {
      ...
      await _client.from('invoice_line_items').insert({
        ...
        'time_entry_id': item.timeEntryId.present
            ? item.timeEntryId.value
            : null,
        'project_id': item.projectId.present ? item.projectId.value : null,
        'issue_reference': item.issueReference.present
            ? item.issueReference.value
            : null,
        'created_at': now,
      });
    }
  ...
  Future<void> deleteDraftInvoice(String invoiceId) async {
    ...
    await _client
        .from('time_entries')
        .update({'is_invoiced': false, 'invoice_id': null, 'updated_at': now})
        .eq('invoice_id', invoiceId);
    await _client
        .from('invoice_line_items')
        .delete()
