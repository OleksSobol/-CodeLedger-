...
class DriftInvoiceRepository implements InvoiceRepository {
  ...
  Stream<List<Invoice>> watchInvoices({String? clientId}) =>
      _dao.watchInvoices(clientId: clientId);
  @override
  Future<Invoice> getInvoice(String id) => _dao.getInvoice(id);
  @override
  Future<List<InvoiceLineItem>> getLineItems(String invoiceId) =>
      _dao.getLineItems(invoiceId);
  @override
  Stream<List<InvoiceLineItem>> watchLineItems(String invoiceId) =>
      _dao.watchLineItems(invoiceId);
  @override
  ...
  Future<void> deleteDraftInvoice(String invoiceId) =>
      _dao.deleteDraftInvoice(invoiceId);
  @override
  Future<bool> archiveInvoice(String invoiceId) =>
      _dao.archiveInvoice(invoiceId);
  @override
  Future<bool> unarchiveInvoice(String invoiceId) =>
      _dao.unarchiveInvoice(invoiceId);
  @override
  Future<void> deleteInvoice(String invoiceId) => _dao.deleteInvoice(invoiceId);
  @override
  Future<void> appendLineItems({
    required String invoiceId,
    required List<InvoiceLineItemsCompanion> lineItems,
    required List<String> timeEntryIds,
  }) => _dao.appendLineItems(
    invoiceId: invoiceId,
    lineItems: lineItems,
    timeEntryIds: timeEntryIds,
  );
  ...
  @override
  Future<bool> revertToDraft(String invoiceId) => _dao.revertToDraft(invoiceId);
  @override
  Future<List<Invoice>> getByStatus(String status) => _dao.getByStatus(status);
  @override
  Future<List<Invoice>> getOverdueInvoices() => _dao.getOverdueInvoices();
  @override
  Future<bool> updateInvoiceNumber(String invoiceId, String invoiceNumber) =>
      _dao.updateInvoiceNumber(invoiceId, invoiceNumber);
  @override
