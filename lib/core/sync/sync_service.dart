...
class SyncStatus {
  ...
  factory SyncStatus.error(String msg, DateTime? lastSyncedAt) => SyncStatus._(
...
class SyncService {
  ...
  SyncService(this._db, this._supabase) : _settings = AppSettingsDao(_db);
            ...
            .insertOnConflictUpdate(
              UserProfilesCompanion(
                ...
                bankAccountType: Value(
                  r['bank_account_type'] as String? ?? 'checking',
                ),
                ...
                paymentInstructions: Value(
                  r['payment_instructions'] as String?,
                ),
                defaultCurrency: Value(
                  r['default_currency'] as String? ?? 'USD',
                ),
                defaultHourlyRate: Value(
                  (r['default_hourly_rate'] as num?)?.toDouble() ?? 0.0,
                ),
                defaultTaxLabel: Value(
                  r['default_tax_label'] as String? ?? 'Tax',
                ),
                defaultTaxRate: Value(
                  (r['default_tax_rate'] as num?)?.toDouble() ?? 0.0,
                ),
                defaultPaymentTerms: Value(
                  r['default_payment_terms'] as String? ?? 'net_30',
                ),
                defaultPaymentTermsDays: Value(
                  r['default_payment_terms_days'] as int? ?? 30,
                ),
                lateFeePercentage: Value(
                  (r['late_fee_percentage'] as num?)?.toDouble(),
                ),
                ...
                invoiceNumberPrefix: Value(
                  r['invoice_number_prefix'] as String? ?? 'INV-',
                ),
            ...
            .insertOnConflictUpdate(
              ClientsCompanion(
                ...
                paymentTermsOverride: Value(
                  r['payment_terms_override'] as String?,
                ),
                paymentTermsDaysOverride: Value(
                  r['payment_terms_days_override'] as int?,
                ),
            ...
            .insertOnConflictUpdate(
              ProjectsCompanion(
                ...
                hourlyRateOverride: Value(
                  (r['hourly_rate_override'] as num?)?.toDouble(),
                ),
            ...
            .insertOnConflictUpdate(
              InvoiceTemplatesCompanion(
                ...
                showTaxBreakdown: Value(
                  r['show_tax_breakdown'] as bool? ?? true,
                ),
                showTaxId: Value(r['show_tax_id'] as bool? ?? true),
                showBusinessLicense: Value(
                  r['show_business_license'] as bool? ?? false,
                ),
                showBankDetails: Value(r['show_bank_details'] as bool? ?? true),
                showStripeLink: Value(r['show_stripe_link'] as bool? ?? false),
                showDetailedBreakdown: Value(
                  r['show_detailed_breakdown'] as bool? ?? true,
                ),
                showPaymentTerms: Value(
                  r['show_payment_terms'] as bool? ?? true,
                ),
                showLateFeeClause: Value(
                  r['show_late_fee_clause'] as bool? ?? false,
                ),
                ...
                lineItemDisplayMode: Value(
                  r['line_item_display_mode'] as String? ?? 'full',
                ),
                createdAt: Value(DateTime.parse(r['created_at'] as String)),
                updatedAt: Value(DateTime.parse(r['updated_at'] as String)),
              ),
        ...
        await _db
            ...
            .insertOnConflictUpdate(
              InvoicesCompanion(
                ...
                lateFeeAmount: Value(
                  (r['late_fee_amount'] as num?)?.toDouble() ?? 0.0,
                ),
                total: Value((r['total'] as num?)?.toDouble() ?? 0.0),
                amountPaid: Value(
                  (r['amount_paid'] as num?)?.toDouble() ?? 0.0,
                ),
                currency: Value(r['currency'] as String? ?? 'USD'),
                notes: Value(r['notes'] as String?),
                templateId: Value(r['template_id'] as String?),
                templateType: Value(
                  r['template_type'] as String? ?? 'detailed',
                ),
            ...
            .insertOnConflictUpdate(
              TimeEntriesCompanion(
                ...
                hourlyRateSnapshot: Value(
                  (r['hourly_rate_snapshot'] as num).toDouble(),
                ),
        ...
        await _db
            ...
            .insertOnConflictUpdate(
              ExpensesCompanion(
                ...
                deductionMethod: Value(
                  r['deduction_method'] as String? ?? 'manual',
                ),
                manualPercentage: Value(
                  (r['manual_percentage'] as num?)?.toDouble(),
                ),
                workHoursPerDay: Value(
                  (r['work_hours_per_day'] as num?)?.toDouble(),
                ),
                totalHoursPerDay: Value(
                  (r['total_hours_per_day'] as num?)?.toDouble(),
                ),
                workSpaceSqft: Value(
                  (r['work_space_sqft'] as num?)?.toDouble(),
                ),
                totalSpaceSqft: Value(
                  (r['total_space_sqft'] as num?)?.toDouble(),
                ),
