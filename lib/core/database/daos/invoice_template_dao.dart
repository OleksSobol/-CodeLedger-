...
class InvoiceTemplateDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<InvoiceTemplate?> getByKey(String key) {
    return (select(
      invoiceTemplates,
    )..where((t) => t.templateKey.equals(key))).getSingleOrNull();
  }

  Future<InvoiceTemplate?> getDefault() async {
    final results =
        await (select(invoiceTemplates)
  ...
  Future<void> setDefault(String id) async {
    await customStatement('UPDATE invoice_templates SET is_default = 0');
    await (update(invoiceTemplates)..where((t) => t.id.equals(id))).write(
      const InvoiceTemplatesCompanion(isDefault: Value(true)),
    );
  }
}
